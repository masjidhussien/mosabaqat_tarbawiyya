function processQuestion(chosenQuestionNumber, transported = 0) {
    ...
    if ((!JSON.parse(sessionStorage.getItem("team1")) || !JSON.parse(sessionStorage.getItem("team2")) || !JSON.parse(sessionStorage.getItem("team1")).name || !JSON.parse(sessionStorage.getItem("team2")).name) && config.style.config.options['use-teams']) return showPopup("warnTeamName");
    // if question transported to other team, play sfx
    document.querySelectorAll("p").forEach(p => {
        p.style.color = invertColor(config.style.config.colors['--question-page-background-color'], true)
    })
    ...
    let countdown = setInterval(() => {
        ...
        backBtn.style.display = choicesContainer.length < 1 ? "block" : "none"
        backBtn.onclick = () => {
            questionsContainer.item(chosenQuestionNumber).setAttribute("chosen", false);
            sessionStorage.setItem("chosenQuestions", sessionStorage.getItem("chosenQuestions") ? sessionStorage.getItem("chosenQuestions").split(",").filter(n => parseInt(n) !== chosenQuestionNumber).join(",") : "")
            returnToQuestions(countdown, 0)
        }
    }, 1000);
