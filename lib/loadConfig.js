...
async function loadConfig() {
    ...
    try {
        const docRef = doc(db, "masjidhussien", "mosabaqat", "quizzes", level);
        ...
        window.config = data;
        let options = data.style.config.options;

        window.questions = data.questions || [];
        window.nashatName = data.style?.nashatName || "مسابقة";

        // Apply Styles (Colors)
        if (data.style?.config.colors) {
            Object.entries(data.style.config.colors).forEach(([k, v]) =>
                document.documentElement.style.setProperty(k, v)
            );
        }
        document.getElementsByClassName("teams")[0].style.display = options['use-teams'] ? "block" : "none"
        TIME_PER_QUESTION = parseInt(options['time-per-question']);
        TRANSFERABLE_QUESTION_TIME = parseInt(options['transferable-question-time']);
        TIME_TO_RETURN = parseInt(options['time-to-return']) * 1000;
        document.getElementsByClassName("logo")[0].style.display = options['show-masjid-logo'] ? "flex" : "none"
        document.getElementsByClassName("qism")[0].style.display = options['show-dept-logo'] ? "flex" : "none"
        document.getElementsByClassName("nashat")[0].style.display = options['show-activity-logo'] ? "flex" : "none"
        // Load Images
...
async function listAllQuizzes() {
    ...
    try {
        const quizzesRef = collection(db, "masjidhussien", "mosabaqat", "quizzes");
